...
const STAGES = [
  ...
  "Grouping repeat merchants",
  "Finding where it's leaking",
...
export function ScanLoading({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    timers.push(setTimeout(onDone, STAGES.length * stepMs + 160));
  ...
  return (
    <div className="flex min-h-[70dvh] flex-col items-center justify-center px-8">
      ...
      >
        ...
        />
        <div className="absolute inset-[30%] rounded-full bg-mint" />
      </motion.div>

      <p className="mb-1 text-xs font-semibold tracking-widest text-mint">SCANNING</p>
